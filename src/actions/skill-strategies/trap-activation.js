...
export class TrapActivationSkillUsage extends SkillUsage {
  constructor(skill) {
    super(skill);
  ...
  trapIsTriggered () {
    const dice = new Dice();
    const roll = dice.rollD(20);
    this.trapRoll = roll;

    return roll < this.effectiveness;
  }
  ...
  endUsage() { 
    if (!this.trapIsTriggered()) return;
  ...
  message () {
    ...
    const title = `A armadilha **${this.skill.name}**(lvl.${this.skill.level}) de **${store.character.name}** foi ativada! ${critText}`
    ...
    const difficulty = `> Dificuldade: ${this.effectiveness} - Resultado do inimigo d20[${this.trapRoll}]`;
...
export default decorate(TrapActivationSkillUsage, {
  startUsage: action,
  endUsage: action,
})