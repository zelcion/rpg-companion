...
export const SkillDisplay = observer(
  class SkillDisplay extends React.Component {
    ...
    addRoundButton() {
      ...
      if (
        this.props.skill.activation === "toggle" ||
        this.props.skill.activation === "charge"
      ) {
        return (
          <button onClick={addHandler} className="main-button spacer">
            {" "}
            Adicionar Turno ({this.skillUsage.rounds}){" "}
          </button>
        );
      }
    }
    ...
    cancelButton() {
      ...
      return (
        <button onClick={this.renewUsage} className="cancel-button spacer">
          {" "}
          Cancelar{" "}
        </button>
      );
    ...
    releaseButton() {
      ...
      if (!this.skillUsage.inUse) return null;
      const buttonText =
        this.props.skill.activation === "trap"
          ? "Ativar Armadilha"
          : "Encerrar Uso";

      return (
        <button onClick={releaseHandler} className="main-button spacer">
          {" "}
          {buttonText}{" "}
        </button>
      );
    ...
    editButton() {
      ...
      return (
        <button className="main-button" onClick={edit} disabled={disabled}>
          {" "}
          Editar{" "}
        </button>
      );
    ...
    render() {
      const effects =
        this.props.skill.effects.length > 0
          ? this.props.skill.effects.join(", ")
          : "nenhum";
      ...
      return (
        <div className={"skill " + inUseClass}>
          <h5>
            ...
            <button onClick={this.levelDown}> - </button> nível{" "}
            {this.props.skill.level} <button onClick={this.levelUp}> + </button>{" "}
          </h5>
          ...
          <button
            onClick={this.skillUsage.startUsage}
            className="main-button spacer"
            disabled={this.skillUsage.inUse}
          >
            {" "}
            Usar{" "}
          </button>
          ...
          {this.cancelButton()}
          {this.editButton()}
          <button className="cancel-button" onClick={this.exclude}>
            {" "}
            Excluir{" "}
          </button>
          <SkillConfigModal
