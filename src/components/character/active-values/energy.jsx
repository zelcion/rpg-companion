...
class EnergyComponent extends React.Component {
  ...
  decreaseValue() {
    this.decreasePrompt.current.begin(value =>
      store.activeValues.decreaseValue("currentEnergy", Number(value)),
    );
  }

  increaseValue() {
    this.increasePrompt.current.begin(value =>
      store.activeValues.increaseValue("currentEnergy", Number(value)),
    );
  }
    ...
    return (
      <div className="energy">
        <FieldPrompt
          ...
          title="Pontos de energia Perdidos"
        />
        <FieldPrompt
          type="number"
          defaultValue={1}
          ref={this.increasePrompt}
          title="Pontos de energia Ganhos"
        />
